...
//converts a byte array into a String that contains the
...
public static void main(String[] args) {
	...
	// Read shadow file provided , save the Usernames, salt, and
	//hashes into memory
	...
	try {
		...
		for (int i=0; i< shadowSimplePassword.length -1; i++) {
			...
			hashesInput[i] = shadowSimplePassword[i].substring(15,
			shadowSimplePassword[i].length());
		}
	
	
		// Read the dictionary file
		String[] dictionaryPasswords = readFile("common-passwords.txt");
		for (int i = 0; i < hashesInput.length-1; i++) {
			for (int j=0; j < dictionaryPasswords.length; j++) {
			
				// Concatenate each dictionary item with each salt
				byte[] saltPasswordInBytes =
				((String)(saltsInput[i] + dictionaryPasswords[j])).getBytes();
				
				//Hash the salt password in bytes and converting
				//it to hex using toHex function
				String saltedHashForPassword =
				toHex(hashGenerator.digest(saltPasswordInBytes));
				
				
				//Print the list of users whose passwords are
				//compromised from the common password file
				if (saltedHashForPassword.compareTo(hashesInput[i]) == 0) {
				System.out.println(usersInput[i] + ":" +
				dictionaryPasswords[j]);
				}
			}
		}
